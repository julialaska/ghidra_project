...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = ghidra_projectFileSystem.MyFileSystemFactory.class)
public class ghidra_projectFileSystem implements GFileSystem {
	 ...
	 */
	public ghidra_projectFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + ghidra_projectFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<ghidra_projectFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public ghidra_projectFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			ghidra_projectFileSystem fs = new ghidra_projectFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
